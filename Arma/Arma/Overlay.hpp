...
inline void __fastcall set_window_to_target()
{
	while (true)
	{
		GameWnd = FindWindowA("Arma 3", NULL);
		if (GameWnd)
		{
...
inline auto __fastcall setup_window() -> void
{
	...
	MyWnd = CreateWindowExA(0, wClass.lpszClassName, ("Steam"), WS_POPUP | WS_VISIBLE, GameRect.left - 2.f, GameRect.top - 2.f, Globals.width + 2.f, Globals.height + 2.f, 0, 0, 0, 0);
...
inline auto __fastcall main_loop() -> WPARAM
{
	...
	while (Message.message != WM_QUIT)
	{
		...
		DrawStrokeText(30, 30, "Arma External");
		crosshair();
